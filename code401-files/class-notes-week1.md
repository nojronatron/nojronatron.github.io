...
- There are 8 primitive data types in Java, everything else *is a class*.
...
- O(n) Non-linear Time => Over time takes increasing amount of time
...
- O(log n): Logarithmic increase in storage space compared to the input size.
- O(n^2): Exponential, inefficient use of space. For every item in the input, additional storage is used.
...
- [X] DS&A Syllabus
- [X] Career Coaching Syllabus
- [X] Take a look at Full Stack Labs.
- [X] Prep for technical moch interviews through reviewing Code Challenges and previous projects.
- [X] Prep for behavioral moch interviews through completion of STAR Questions.
- [X] Study Whiteboard Challenge Workflow to get your future whiteboards to look a certain way.
- [X] Not sure how to do something like build a project? Run it? Do it over and over until it makes sense and becomes easier and commits to memory.
- [X] Include blockers and challenging areas in Learning Journals for your own good *and* so Alex is aware of what is going on.
- [X] Ask yourself DS&A questions: How do I traverse this structure? Sort that out *first*.
- [X] Thursday Code Challenges are *Mock Interview* assignments, timeboxed to 30 minutes.
- [X] Build a shortcut to assist with string concatenation in Java (`String.concat(stringOne).concat(stringTwo)`, `StringBuilder` class, `String.format("%s %n", stringOne)`, `String.join(str1, str2, ...)`).
- [X] GitIgnore: Use this for the *entire class*
...
- [X] Complete Lab03
- [X] Lab04 (make as much progress as possible within timebox TBD) (interact with a BMP file)
- [X] Learning Journal
